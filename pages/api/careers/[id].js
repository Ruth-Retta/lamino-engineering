...
export default async function handler(req, res) {
  ...
  if (req.method === "GET") {
    try {
      ...
      res
        .status(500)
        .json({ message: "Failed to fetch career", error: error.message });
    }
  } else {
    ...
    if (req.method === "PUT") {
      ...
      form.parse(req, async (err, fields, files) => {
        ...
        try {
          ...
          careerResponse.imageId = updatedCareer.image
            ? updatedCareer._id
            : null;
          delete careerResponse.image;
          ...
          res
            .status(400)
            .json({ message: "Failed to update career", error: error.message });
        }
      });
      ...
      try {
        ...
        res
          .status(500)
          .json({ message: "Failed to delete career", error: error.message });
      }
    } else {
